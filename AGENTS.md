...
- **续接顺序**：①读本文件 → ②`design_status` 刷基 → ③`design_get` `DEC-OPI-5ba74efc-9f26-483b-a7de-c833834275a4.115`（**P4-1 处置+待裁裁定**：A=key 合规/B=附条件（**P4-2 必带 turn→summary 装配校验**）；违规第五次升级+落盘自查清单机制）→ ④`git log --oneline` → ⑤开工前读 `docs/IMPLEMENTATION_PLAN.md` §4（Phase 4 Detail Block）与 §1.5，再按派单协议铸下一刀任务书（四查+VAL+TASK 同铸；**四查必答：列名/词表照 canonical 原文；P4-2 任务书必带 DEC-…115 三项（turn→summary 装配校验/F1 refs↔proposal 对账扫描/F3 收窄）+落盘通道自查清单**）
