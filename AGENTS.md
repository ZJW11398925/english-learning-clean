...
- **续接顺序**：①读本文件 → ②`design_status` 刷基 → ③`design_get` 三件：`DEC-OPI-4d516e4f-9575-4cbf-8d08-6bf19a95bd25.29`（**P4-4 处置 + PHASE 4 COMPLETE 收口**：五刀链+五验收+遗留清单七项）＋`DEC-…5ba74efc.68`（**Phase 4 总设计权威**，其表述收窄句 Phase 5 仍要）＋`PLAN-…5ba74efc.82`（Phase 4 母计划，已全毕）→ ④`git log --oneline` → ⑤开工前读 `docs/IMPLEMENTATION_PLAN.md` §1.5 与 **§6（Phase 5 Detail Block）**，再按派单协议铸下一刀任务书（四查+VAL+TASK 同铸；**四查必答：列名/词表照 canonical 原文；Phase 5 首刀必带 DEC-…29 遗留清单取舍（①`derive_reply` 启发式修法/③prompt 转义归 Phase 9/④Episode status/⑤双证口径/⑥history 期望/⑦零候选边界）+两份清单（落盘通道自查+Bash 使用清单）；**VAL 一律带实际触及路径集**）
