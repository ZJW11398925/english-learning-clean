...
- **续接顺序**：①读本文件 → ②`design_status` 刷基 → ③`design_get` 三件：`DEC-OPI-4d516e4f-9575-4cbf-8d08-6bf19a95bd25.13`（**P4-2 处置+revisit**：P4-3 必带四项）＋`DEC-…5ba74efc.68`（**Phase 4 总设计权威**：P4-G1 门定义+五刀切法+P4-0 两 carryover+projection 语义要求——P4-3/4 全都要它）＋`PLAN-…5ba74efc.82`（Phase 4 母计划）→ ④`git log --oneline` → ⑤开工前读 `docs/IMPLEMENTATION_PLAN.md` §4（Phase 4 Detail Block）与 §1.5，再按派单协议铸下一刀任务书（四查+VAL+TASK 同铸；**四查必答：列名/词表照 canonical 原文；P4-3 任务书必带 DEC-…13 revisit 四项（SUPPORTED_PROJECTION_TYPES 扩集（EPISODE）时重审 `ensure_projection_job`/`ensure_missing_jobs` 单 id 返回形状与 `turns_missing_projection` 多类型读 / F-5 claim 集合 discard / F1 对账 N+1 规模面 / 下次 submit 补 evidence_basis 族字段）+落盘通道自查清单；**VAL 一律带实际触及路径集**（VAL-…5ba74efc.74 路径集过窄的先例）**）
- **执行者遗留处理先例**（发生过两次：P3-1A/P3-2 前均因 API 余额中断留下半成品）：开工前必查 git status——不干净时先取证（快照/差异分析），按「吸收+逐文件复核+修正+补齐+留痕」或「拒收重交」二选一，绝不静默覆盖
- **当前状态**（2026-09-21 第九会话末）：**Phase 4 进行中，P4-0+P4-1+P4-2 COMPLETE**（rev 71；sync 见收口记录）——P4-0 `d8866b8`（`VR-…5ba74efc.91`）+P4-1 `2acaaca`（`VR-…5ba74efc.109`）+P4-2 `ffcafcf`（`VR-OPI-4d516e4f-…12` PASS，处置 `DEC-OPI-4d516e4f-…13`）。P4-2 CP4 projection runtime 全落：`runtime/projections.py`（确定性 `pj-` id/幂等 enqueue/五态 CAS 全边矩阵/slice hash+base 双重重校验（不匹配→REJECTED 无 blind replay）/crash-gap `ensure_projection_job`/启动三连 **reopen→ensure→drain**/进程内 claim 集合护栏/recover_stale_running）+`platform/db/projection_store.py`（**出生只许 PENDING**；五态 CAS；join 读）+`relationship/projection.py`（装配一致性校验=DEC-…115 待裁 B 附条件兑现；不自行判类；refusals 不阻断）+coordinator **guard 释放后**投影线（`begin_turn`/`respond_to_teaching` 拆 `*_guarded` 逐字节等价）+启动恢复五步 +**F3 按 BF-05 §29 收窄**（跨 persona supersede 与不存在同一条拒词，无 id 回显）+**F1/F2 恢复线**（teaching refs 全量扫描面+全 ref 判存在→悬空报告；pending evidence retry 接线且异常收口）。评审链：独立评审 A–G 七面全 PASS+20 条自建探针（含 **585 条既有断言以 HEAD 快照双侧对跑证明零放松**）；findings 2 MINOR+3 LOW+5 INFO 全处置（F-1 出生 PENDING/F-2 启动抽干在册 backlog/F-4 门面指针/F-5 claim 集护栏+四项文书求真）；执行器面 `project()` 命名（**去绕门**：曾以「取方法值再调用」绕 Gate 2，总控判返工改正向命名）。测试读数 **639/0/0**、mypy 103 文件、migrations 0009/v9、零 schema 变更。**下一步 = P4-3**（Episode projection+DisclosedUserProfile+PromptCompiler 消费 `RelationshipView`，VAL-…5ba74efc.76；**铸书必带 DEC-…13 revisit 四项+落盘自查清单+VAL 带实际路径集**）→ P4-4（stress+端到端验收，VAL-…80）→ Phase 4 收口
- **对象索引**（全 id 可直接 design_get；OPI 前缀：Phase 3 早期 `2babb21e-bc72-47a2-9382-e773c92211d2`、P3-1B 起及 Phase 4 全在 `5ba74efc-9f26-483b-a7de-c833834275a4`；旧系列 `eaaa5a1d-7ac7-4746-bcdf-c02bc9147492`）：
  - `DEC-…4d516e4f.13` **P4-2 处置+revisit**（P4-3 必带四项：EPISODE 扩集重审单 id 返回形状 / F-5 集合 discard / F1 N+1 规模面 / evidence_basis 族字段补录）——P4-3 续接先读
  - `DEC-…5ba74efc.115` P4-1 处置+待裁裁定（其 P4-2 必带三项已由 P4-2 全兑现）；`DEC-…5ba74efc.96` P4-0 处置；`DEC-…5ba74efc.68` **外部评审采纳（Phase 3 复核+P4-0 授权）**
  - `PLAN-…5ba74efc.82` Phase 4 母计划（p4-0..p4-4）；`VAL-…5ba74efc.70/.72/.74/.76/.80` 五验收（**VAL-…74 路径集过窄未被单独提交**，其同题四组并入 `VAL-OPI-4d516e4f.…7` 盖章——取代关系见 `DEC-…4d516e4f.13`）
  - `TASK-…5ba74efc.84/.100`+`VR-…5ba74efc.91/.109` P4-0/P4-1 两对（PASS）；`TASK-OPI-4d516e4f.…9`+`VR-OPI-4d516e4f.…12`+`VAL-OPI-4d516e4f.…7` P4-2 三件（PASS）
...
- **测试**：`python -m pytest`（当前 639/0/0）；静态：ruff + mypy（103 文件）；migrations 到 0009（schema_version 9）；GenerationAction 权威在 runtime（DM §16）；DecisionCycle 权威在 Runtime（`DEC-…2babb21e.5`）；TeachingMoment/Attempt 全链已实装（P3-1/2/3）；Relationship durable core 已实装（P4-1）；**CP4 projection runtime 已实装（P4-2：权威面 `elc.runtime.projections`（SQL-free，执行器协议方法名 `project()`）+ SQLite 适配器 `elc.platform.db.projection_store`；`projection_job` 表自 P4-2 起有真实读写者，此前为空）**
...
- **VAL 路径集纪律**（P4-2 教训留痕）：验收的 `applicability_contract.paths` 必须覆盖实际触及路径——VAL-…74 漏 `src/elc/platform/db`（Gate 2 规定 runtime 无 SQL，Runtime 拥有的投影适配器只能落此）与 `src/elc/teaching`（对账需 refs 扫描面），P4-2 铸同题扩集 `VAL-OPI-4d516e4f.…7` 盖章并在 `DEC-…4d516e4f.13` 显式留痕取代关系（**不双重声明同一观察**）
...
- **不做**（纪律）：未到位次的域逻辑/域表提前实现（位次见基线 §1.5：Phase 4=Relationship+Episode projection 进行中（P4-0 契约/P4-1 Relationship 写入面/P4-2 CP4 projection runtime 均已落地；**Episode projection 与 DisclosedUserProfile/PromptCompiler 消费=P4-3**）；natural-conversation silent Evidence 留 Phase 5；其余域仍禁）；Redis/Kafka/分布式锁/TTL/heartbeat（Local V1 §24.1 永禁）；自动教学/Planner SELECT（Phase 8 才开）；`decision_cycle_id` 收紧已随 0007 兑现（`DEC-…091f35c3.7` 裁定 b 闭环）
