...
- **续接顺序**：①读本文件 → ②`design_status` 刷基 → ③`design_get` 三件：`DEC-OPI-5ba74efc-9f26-483b-a7de-c833834275a4.115`（**P4-1 处置+待裁裁定**）＋`DEC-…5ba74efc.68`（**Phase 4 总设计权威**：P4-G1 门定义+五刀切法+P4-0 两 carryover+projection 语义要求——P4-2/3/4 全都要它）＋`PLAN-…5ba74efc.82`（Phase 4 母计划）→ ④`git log --oneline` → ⑤开工前读 `docs/IMPLEMENTATION_PLAN.md` §4（Phase 4 Detail Block）与 §1.5，再按派单协议铸下一刀任务书（四查+VAL+TASK 同铸；**四查必答：列名/词表照 canonical 原文；P4-2 任务书必带 DEC-…115 三项（turn→summary 装配校验/F1 refs↔proposal 对账扫描/F3 收窄）+落盘通道自查清单**）
- **执行者遗留处理先例**（发生过两次：P3-1A/P3-2 前均因 API 余额中断留下半成品）：开工前必查 git status——不干净时先取证（快照/差异分析），按「吸收+逐文件复核+修正+补齐+留痕」或「拒收重交」二选一，绝不静默覆盖
- **当前状态**（2026-09-21 第八会话末）：**Phase 4 进行中，P4-0+P4-1 COMPLETE**（rev 68，sync REBUILD+CHECK 零告警）——P4-0 `d8866b8`（`VR-…5ba74efc.91`）+P4-1 `2acaaca`（`VR-…5ba74efc.109`）：Relationship durable core 全落——Recorder（P4-G1 门=行为断言级：只消费 CanonicalTurnSlice+SamePersonaExistingRelationshipSummary/key 字段集钉死/command turn 过滤 fail-closed/未交付不入/三词全点名钉）/validate 八规则/dedupe 规范化精确匹配/append-first supersede（同短事务+原子性探针）/sensitive gate 四判定序（模型不能自授许可）/Persona×User 全隔离/RelationshipView+Summary 最小查询面。评审 5 findings 全 INFO/LOW 处置 `DEC-…5ba74efc.115`（**P4-2 必带**：turn→summary 装配校验（待裁 B 附条件）、F1 refs↔proposal 对账扫描、F3 按删除契约 §29 收窄；F1 文案/F5 随下次触碰）；第五次 Bash 违规（自报+整文件 Write 重写返工）升级留痕+**落盘通道自查清单机制**（P4-2 起任务书要求执行者逐文件列通道）；测试读数 585/0/0、mypy 100 文件、migrations 0009/v9。**下一步 = P4-2**（CP4 projection runtime：enqueue_projection 去 stub/stable id+idempotent/PENDING→RUNNING→COMMITTED/FAILED_RETRYABLE→REJECTED/source_turn_slice_hash+base_domain_version revalidation/retry/CP4 不持 coordinator guard/三不变量（不回滚 transcript/不重发 assistant/不阻下轮）/crash-gap ensure_projection_job 确定 id 补齐；**必带 DEC-…115 三项+落盘自查清单**；验收=VAL-…5ba74efc.74）→ P4-3（Episode+DisclosedUserProfile+PromptCompiler 消费，VAL-…76）→ P4-4（stress+端到端验收，VAL-…80）→ Phase 4 收口
- **对象索引**（全 id 可直接 design_get；OPI 前缀：Phase 3 早期 `2babb21e-bc72-47a2-9382-e773c92211d2`、P3-1B 起及 Phase 4 全在 `5ba74efc-9f26-483b-a7de-c833834275a4`；旧系列 `eaaa5a1d-7ac7-4746-bcdf-c02bc9147492`）：
  - `DEC-…5ba74efc.115` **P4-1 处置+待裁裁定**（P4-2 必带三项：turn→summary 装配校验/F1 对账扫描/F3 收窄；落盘自查清单机制）——P4-2 续接先读
  - `DEC-…5ba74efc.96` P4-0 处置；`DEC-…5ba74efc.68` **外部评审采纳（Phase 3 复核+P4-0 授权）**
  - `PLAN-…5ba74efc.82` Phase 4 母计划（p4-0..p4-4）；`VAL-…5ba74efc.70/.72/.74/.76/.80` 五验收
  - `TASK-…5ba74efc.84/.100`+`VR-…5ba74efc.91/.109` P4-0/P4-1 两对（PASS）
...
- **测试**：`python -m pytest`（当前 585/0/0）；静态：ruff + mypy（100 文件）；migrations 到 0009（schema_version 9）；GenerationAction 权威在 runtime（DM §16）；DecisionCycle 权威在 Runtime（`DEC-…2babb21e.5`）；TeachingMoment/Attempt 全链已实装（P3-1/2/3）；Relationship durable core 已实装（P4-1：Recorder/validate-dedupe/append-first supersede/sensitive gate/Persona×User 隔离/查询面）；P4-2 起 CP4 projection runtime
