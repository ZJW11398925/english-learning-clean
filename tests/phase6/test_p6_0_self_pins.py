...
- **the fixtures build the world through the shipped chain.** The phase 6
  conftest contains no SQL write statement at all — its app.db comes from
  ``elc.platform.db.migrations`` and its rows from the stores under test, so
  a hand-written schema or a hand-inserted row cannot pass for the shipped
  one.
"""
