...
# 13×None; C2-a: 9×R4 + 5×None; C2-b: 28×R4 + 5×None), and the block is named
# ---------------------------------------------------------------------------


def test_five_targets_read_no_level_and_twenty_eight_read_r4(
    built_content_db: Path,
) -> None:
    """Old truth: all fourteen read no level, blocked at R0 by
    ``assessment_membership``. New truth (C1 one target, C2-a the other
    eight, C2-b the nineteen it authored as entities): the five CAPABILITY
    entities whose sources state no evidence read exactly that, and the
    twenty-eight RESOURCE targets whose sources state all nineteen facts read
    ``R4_DETECTION_READY`` — the same fail-closed ladder, now driven by the
    artifact."""
