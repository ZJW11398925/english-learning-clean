...
def test_validation_run_leaves_the_evidence_kernel_untouched(
    ...
    outcomes = []
    for text, expect_abstain_like in (
        ("", False),  # no output → RETRY
        ("You have mastered this expression completely!", False),
        ("Sure — let's practice that phrase.", False),
    ):
        del expect_abstain_like
...
def test_persona_validator_decision_face_has_no_evidence_vocabulary() -> None:
    ...
    # make_claim's PerformanceType import stays valid: the learning
    # vocabulary and the validator vocabulary are disjoint worlds.
