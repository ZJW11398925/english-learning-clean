...
def _critical_unknown(req):
    unknown=[]
    fields=[
        ("decision_cycle_status", {"VALID","INVALIDATED","STALE","UNKNOWN"}),
...
def decide(req):
    ...
    if req["decision_cycle_status"] in {"INVALIDATED","STALE"}:
        reasons.append("DECISION_CYCLE_INVALID")
    ...
    # Latest user intent is revalidated at execution time.
    ...
    # Auto-teach preference applies only to genuinely automatic consent.
    ...
    # Flow protection prevents automatic intrusion, not an explicit new request.
    if (
        (gc=="OPEN" and ap=="AUTOMATIC")
        or gc=="AUTO_CONTINUE"
    ) and req.get("hard_protected_flow",False):
        reasons.append("HARD_PROTECTED_FLOW")

    # Automatic-opening-only controls.
    if gc=="OPEN" and ap=="AUTOMATIC":
    ...
    # Continuation caps. Terminalizing actions remain possible so the moment can close cleanly.
    terminalizing = (
    ...
    ordered=[r for r in DENY_PRECEDENCE if r in reason_set]
